...
u32 pico_openNdzVirtual(const void* payload)
{
    ...
    card_romStartXfer(MCCNT1_DIR_WRITE | MCCNT1_RESET_OFF | MCCNT1_CLK_6_7_MHZ |
        ...
        MCCNT1_CLOCK_SCRAMBLER | MCCNT1_READ_DATA_DESCRAMBLE | MCCNT1_LATENCY1(0), false);
    if ((u32)payload & 3)
        card_romCpuWriteUnaligned((const u8*)payload, 128);
    else
...
void pico_closeNdzVirtual(void)
{
    card_romSetCmd(DSPICO_CMD_CLOSE_NDZ_VIRTUAL);
    card_romStartXfer(MCCNT1_DIR_READ | MCCNT1_RESET_OFF | MCCNT1_CLK_6_7_MHZ |
        MCCNT1_LEN_0 | MCCNT1_CMD_SCRAMBLE | MCCNT1_LATENCY2(0) |
        MCCNT1_CLOCK_SCRAMBLER | MCCNT1_READ_DATA_DESCRAMBLE | MCCNT1_LATENCY1(0), false);
    card_romWaitBusy();
}

// read from the open .ndz: E3 with bit 32 set, then E4 and E5
bool pico_ndzReadDecompressed(u8* dst, u32 offset, u32 length)
{
    u8 bounce[512] __attribute__((aligned(4))); // on the stack so the driver fits 2 KB
    while (length)
    {
        u32 inSector = offset & 0x1FFu;
        u32 take = 512u - inSector;
        if (take > length) take = length;
        card_romSetCmd(0xE300000100000000ull | (offset >> 9));
        card_romStartXfer(MCCNT1_DIR_READ | MCCNT1_RESET_OFF | MCCNT1_CLK_6_7_MHZ |
            MCCNT1_LEN_0 | MCCNT1_CMD_SCRAMBLE | MCCNT1_LATENCY2(0) |
            MCCNT1_CLOCK_SCRAMBLER | MCCNT1_READ_DATA_DESCRAMBLE | MCCNT1_LATENCY1(0), false);
        card_romWaitBusy();
        while (!pollSdDataReady());
        getSdData(bounce);
        for (u32 i = 0; i < take; i++) dst[i] = bounce[inSector + i];
        dst    += take;
        offset += take;
        length -= take;
    }
